       ...
       <!--PAGE CONTENT -->
        <div id="content">
                ...
                <div class="inner">
                    <div class="row">
                <div class="col-lg-12">
                    <h2 class="page-header">แก้ไขข้อมูลการนัดหมาย</h2>
                </div>
            </div>
<div class="row">
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>กรอกรายละเอียดข้อมูลการนัดหมาย</h5>
        </header>
        <div id="div-1" class="accordion-body collapse in body">
        <?
			$strSQL = "SELECT * FROM calendar where calendar_id = '$_GET[calendar_id]'  ";
			$stmt  = mysqli_query($conn,$strSQL);
			$row = mysqli_fetch_array($stmt);
		?>
            <form method="post" action="update_calendar.php" class="form-horizontal" enctype="multipart/form-data">
            <input type="hidden" name="calendar_id"  value="<?=$row[calendar_id];?>">
              <h4 class="form-signin-heading" align="center"></h4>
                      ...
                      <div class="col-sm-5">
                          <select name="club_id" class="form-control" required>
                            <?
							  ...
							  ?>
                                <option value="<?=$row_club[club_id];?>" <? if($row_club[club_id] == $row[club_id]){echo 'selected'; } ?>><?=$row_club[club_name];?></option>
                            <?  } ?>
                          </select>
                      </div>
                  </div>
                  <div class="form-group">
                      ...
                      <div class="col-sm-3">
                           <input class="form-control" type="date" name="calendar_date" min="<?=date('Y-m-d'); ?>" value="<?=date('Y-m-d', strtotime($row[calendar_date]));?>" required>
                      </div> <B><font color="red" size="5" > * </font></B>
                  </div>
                  <div class="form-group">
                      <label class="control-label col-sm-3">เวลา :</label>
                      <div class="col-sm-3">
                           <input class="form-control" type="time" name="calendar_time" value="<?=date('H:i', strtotime($row[calendar_time]));?>" required>
                      </div>
