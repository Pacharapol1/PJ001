       ...
       <!--PAGE CONTENT -->
        <div id="content">
                ...
                <div class="inner">
                    <div class="row">
                <div class="col-lg-12">
                    <h2 class="page-header">แก้ไขข้อมูลคณาจารย์</h2>
                </div>
            </div>
...
<div class="col-lg-12">
    <div class="box dark">
		...
		?>
            <form method="post" action="update_emp.php" class="form-horizontal" enctype="multipart/form-data">
            <input type="hidden" class="form-control" name="emp_id" value="<?=$row[emp_id];?>" required>
              <h4 class="form-signin-heading" align="center"></h4>
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-sm-2">
                          <select name="title_id" class="form-control" required>
                            <?
								$sql_title = "SELECT * FROM titlename where title_status != 'C' ORDER BY title_name ASC ";
								$stm_title = mysqli_query($conn,$sql_title);
								while($row_title = mysqli_fetch_array($stm_title)){
							?>
                            		<option value="<?=$row_title[title_id];?>" <? if($row_title[title_id] == $_GET[title_id]){echo 'selected'; } ?>><?=$row_title[title_name];?></option>
                            <?	} ?>
                          </select>
                      </div>
                      <div class="col-sm-3">
                      		<input type="text" class="form-control" placeholder="ชื่อ" name="emp_name" value="<?=$row[emp_name];?>" required>
                      </div>
					  <div class="col-sm-3">
                      		<input type="text" class="form-control" placeholder="นามสุกล" name="emp_surname" value="<?=$row[emp_surname];?>" required>
                      </div>
                  </div>
                  ...
                  <div class="form-group">
                      ...
                      <option value="ชาย" <? if($row[mem_sex] == 'ชาย'){echo 'selected'; } ?>>ชาย</option>
                      <option value="หญิง" <? if($row[mem_sex] == 'หญิง'){echo 'selected'; } ?>>หญิง</option>
                    </select>
                      </div>
                  </div>
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-sm-3">
                          <input type="text" class="form-control" placeholder="เบอร์โทรศัพท์" maxlength="10" value="<?=$row[emp_mobile];?>" name="emp_mobile" OnKeyPress="return chkNumber(this)" required>
                      </div>
                  </div>
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-sm-3">
                          <input  type="text" class="form-control" placeholder="ที่อยู่" name="emp_num" value="<?=$row[emp_num];?>" required>
                      </div>
                  </div>
					   <div class="form-group">
                      <label class="control-label col-sm-3">ตำบล :</label>
                      <div class="col-sm-3">
                          <input  type="text" class="form-control" placeholder="ตำบล" name="emp_tum" value="<?=$row[emp_tum];?>" required>
                      </div>  
                  </div>
					   <div class="form-group">
                      <label class="control-label col-sm-3">อำเภอ :</label>
                      <div class="col-sm-3">
                          <input  type="text" class="form-control" placeholder="อำเภอ" name="emp_aum" value="<?=$row[emp_aum];?>" required>
                      </div>  
                  </div>
					   <div class="form-group">
					 ...
					 <div class="form-group">
                      ...
                      <div class="col-sm-3">
                          <input  type="text" class="form-control" placeholder="รหัสไปรษณีย์" name="emp_zip" value="<?=$row[emp_zip];?>" required>
                      </div>  
                  </div> 
                  <div class="form-group">
