       ...
       <!--PAGE CONTENT -->
        <div id="content">
                ...
                <div class="inner">
                    <div class="row">
                <div class="col-lg-12">
                    <h2 class="page-header">แก้ไขประกาศการสัมภาษณ์</h2>
                </div>
            </div>
...
<div class="col-lg-12">
    <div class="box dark">
        <header>
            ...
            <h5>กรอกข้อมูลประกาศการสัมภาษณ์</h5>
        </header>
        <?
			$strSQL = "SELECT * FROM interview where interview_id = '$_GET[interview_id]' ";
			$stmt  = mysqli_query($conn,$strSQL);
			$row = mysqli_fetch_array($stmt);
		?>
        <div id="div-1" class="accordion-body collapse in body">
            <form method="post" action="update_interview.php" class="form-horizontal" enctype="multipart/form-data">
              ...
              <h4 class="form-signin-heading" align="center"></h4>
                  ...
                  <div class="form-group">
                      ...
                      <div class="col-sm-3">
                           <input class="form-control" type="date" name="interview_date" value="<?=$row[interview_date];?>" required>
                      </div> 
                  </div>
                  <div class="form-group">
                      <label class="control-label col-sm-3">เวลานัดสัมภาษณ์  :</label>
                      <div class="col-sm-3">
                           <input class="form-control" type="time" name="interview_time" value="<?=$row[interview_time];?>" required>
                      </div> 
                  </div>
                  <div class="form-group">
                      <label class="control-label col-sm-3">สถานที่  :</label>
                      <div class="col-sm-5">
                           <input class="form-control" type="text" name="interview_place" value="<?=$row[interview_place];?>" required>
                      </div> 
                  </div>
                  <div class="form-group">
                      <label class="control-label col-sm-3">รายละเอียด  :</label>
                      <div class="col-sm-5">
                           <textarea name="interview_detail" class="form-control" rows="3"><?=$row[interview_detail];?></textarea>
                      </div> 
                  </div>
                  <div class="form-group">
                      <label class="control-label col-sm-3">วันที่ประกาศ  :</label>
                      <div class="col-sm-3">
                           <input class="form-control" type="date" name="interview_date_start" min="<?=date('Y-m-d');?>" value="<?=$row[interview_date_start];?>" required>
                      </div> 
                  </div>
                  <div class="form-group">
                      <label class="control-label col-sm-3">วันที่สิ้นสุด  :</label>
                      <div class="col-sm-3">
                           <input class="form-control" type="date" name="interview_date_end" min="<?=date('Y-m-d');?>" value="<?=$row[interview_date_end];?>" required>
                      </div> 
                  </div>
                  <div class="form-group">
