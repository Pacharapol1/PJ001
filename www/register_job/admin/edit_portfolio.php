       ...
       <!--PAGE CONTENT -->
        <div id="content">
                ...
                <div class="inner">
                    <div class="row">
                <div class="col-lg-12">
                    <h2 class="page-header">แก้ไขข้อมูลการดำเนินโครงการ</h2>
                </div>
            </div>
...
<div class="col-lg-12">
    <div class="box dark">
        ...
        <div id="div-1" class="accordion-body collapse in body">
            <form method="post" action="update_portfolio.php" class="form-horizontal" enctype="multipart/form-data">
              <input type="hidden" name="portfolio_id" value="<?=$row[portfolio_id];?>">
              <h4 class="form-signin-heading" align="center"></h4>
                  <!--<div class="form-group">
                      <label class="control-label col-sm-3">ชื่อภาพกิจกรรม :</label>
                      <div class="col-sm-5">
                          <input type="text" class="form-control" placeholder="ชื่อภาพกิจกรรม" value="<?=$row[portfolio_name];?>" name="portfolio_name" required>
                      </div>
                  </div>-->
                  <div class="form-group">
                  <label class="control-label col-sm-3">ชื่อโครงการ :</label>
                   <div class="col-sm-7">
                       <select name="calendar_id" class="form-control" required>
                            <?
							$sql_ca = "SELECT * FROM calendar where calendar_status != 'C' and calendar_status != 'P' ORDER BY calendar_name ASC ";
							$stm_ca = mysqli_query($conn,$sql_ca);
							while($row_ca = mysqli_fetch_array($stm_ca)){
						  ?>
                                <option value="<?=$row_ca[calendar_id];?>" <? if($row_ca[calendar_id] == $row[calendar_id]){echo 'selected'; } ?>><?=$row_ca[calendar_name];?></option>
                            <?  } ?>
                          </select>
                  </div>
                  </div>
                  <div class="form-group">
                      ...
                      <div class="col-sm-5">
                      	   <a href="<?=$row[portfolio_files];?>" target="_blank" class="btn btn-default btn-sm"><i class="icon-folder-open"></i> เอกสาร</a><br><br>
                           <input type="hidden" name="hdnOldFile" value="<?=$row[portfolio_files];?>">
                           <input class="form-control" id="fileInput" type="file" name="applicant_files" >
                      </div> 
                  </div>
                  <div class="form-group">
                    ...
                    <div class="col-sm-6">
						...
						?>
                            <tr>
                                ...
                                <? } ?>
                                <input type="text" name="hdnOldFile<?=$num;?>" value="<?=$row_pic[portfolio_pic];?>">
