       ...
       <!--PAGE CONTENT -->
        <div id="content">
                ...
                <div class="inner">
                    <div class="row">
                <div class="col-lg-12">
                    <h2 class="page-header">รายงานบันทึกรายงานประจำวัน</h2>
                </div>
            </div>
...
<div class="col-lg-12">
    <div class="box dark">
        ...
        <div id="div-1" class="accordion-body collapse in body">
            <form method="post" action="exp_work_report.php" class="form-horizontal" enctype="multipart/form-data">
              <h4 class="form-signin-heading" align="center"></h4>
                  ...
                  <div class="form-group">
                      <label class="control-label col-sm-3">รายชื่อนักศึกษา :</label>
                      <div class="col-sm-6">
                      		<select name="mem_id" class="form-control" required>
                            <option value="">เลือกรายชื่อ</option>
                            <?
								$sql_club = "SELECT * FROM member 
									inner join diary on diary.mem_id = member.mem_id
									inner join titlename on titlename.title_id = member.title_id
									where member.mem_status != 'C'
									group by diary.mem_id
									ORDER BY mem_name ASC ";
								$stm_club = mysqli_query($conn,$sql_club);
								while($row_club = mysqli_fetch_array($stm_club)){
							  ?>
                                <option value="<?=$row_club[mem_id];?>" <? if($row_club[mem_id] == $_GET[mem_id]){echo 'selected'; } ?>><?=$row_club[title_name];?><?=$row_club[mem_name];?> <?=$row_club[mem_last];?></option>
                            <?  } ?>
                          </select>
                      </div>
                  </div>
