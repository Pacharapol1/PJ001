...
<div class="col-lg-12">
    <div class="box dark">
        ...
        <div id="div-1" class="accordion-body collapse in body">
        	<?
				$strSQL = "SELECT * FROM tb_file where file_id = '".$_GET[file_id]."'";
				$stmt  = mysqli_query($conn, $strSQL);
				$row = mysqli_fetch_array($stmt);
			?>
            <form method="post" action="update_file.php" class="form-horizontal" enctype="multipart/form-data">
            <input type="hidden" name="file_id" value="<?=$_GET["file_id"];?>">
            <input type="hidden" name="i" value="<?=$_GET[file_file];?>">
              <h4 class="form-signin-heading" align="center"></h4>
                  <div class="form-group">
                      <label class="control-label col-sm-4"><?=$namebook[$_GET[file_file]];?> :</label>
                      <div class="col-sm-5">
                      		<a href="<?=$row[file_file.$_GET[file_file]];?>" target="_blank">เอกสารเดิม</a>
                            <input type="hidden" name="hdnOldFile" value="<?=$row[file_file.$_GET[file_file]];?>">
                          	<input class="form-control" type="file" name="file_file<?=$_GET[file_file];?>" required>
                      </div>
